...
- `/` Landing Page
- `/login` Mock login or Real Bridge login
- `/register` Mock registration or Real Bridge registration through UserService
- `/app/chat` IM client with conversation list, single chat, group chat, ACK states and mock WebSocket
...
- `/app/settings` Mock/Real Bridge mode, Gateway URL, WebSocket URL and local data controls
- `/dashboard` Mock monitoring dashboard
- `/admin` AdminService console through the Web Bridge HTTP proxy

## Features

- Mock API layer under `src/api`
- Mock WebSocket / Gateway abstraction under `src/services`
...
- Simulated random send failures
- Simulated Gateway connection, heartbeat and latency
...
- Playwright E2E smoke flow for Mock login and chat send
...
The current implementation uses Mock API + Mock WebSocket:
...
Mock users include Alice, Bob, Charlie and Diana. Mock conversations include Alice, Bob, Charlie, Nebula Core Team and Backend Infra Group. Mock metrics cover Gateway connections, online users, message QPS, push success rate, Kafka consume rate and P99 latency.
...
- Replace Mock API implementations in `src/api`.
...
Mock mode does not require a backend:
...
6. Switch to Real Bridge mode:

```text
Settings -> Connection Mode = Real Bridge
...
- Login works in Mock but not Real: verify backend UserService and Gateway auth protocol.
- Sending to mock users in Real mode fails: Real mode requires numeric backend `user_id` / `group_id`. Add a numeric user ID in Contacts before opening a real single chat.
